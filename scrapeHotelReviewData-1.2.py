...
def analyzeReviewPageModified(hotelid, contents, hName, nextpath):
    ...
    """
    print("In analyze reviews page modified.")
    select = pq(contents)
    ...
    while count < numRatings:
        fileToOpen = nextpath + '/'  + str(count) + '.txt'
        stringToWrite = ''
        outfile = open(fileToOpen, 'w')
        # reviewer details
        reviewerName = getTextIfItIsThere(pq(ratingElements[count])('.username'))
        stringToWrite += 'Reviewer Name: ' + reviewerName + '\n'
        #outfile.write(reviewerName + '\n')
        #title = pq(pq(ratingElements[count])('.reviewerTitle')[0]).text()
        #outfile.write(title + '\n')
        #reviewerId = '213123123'        ###
        placeOfResidence = getTextIfItIsThere(pq(ratingElements[count])('.location'))
        stringToWrite += 'Place: ' + placeOfResidence + '\n'
        #outfile.write(placeOfResidence + '\n')
        badgeText = ''.join(pq(ratingElements[count])('.badgeText').text())
        stringToWrite += 'Badges: ' + badgeText + '\n'
        #outfile.write(badgeText + '\n')
        ...
        stringToWrite += '\n'
        #outfile.write('\n')
        # create the string with this data.
        title = getTextIfItIsThere(pq(ratingElements[count])('.noQuotes'))
        stringToWrite += 'title: ' + title + '\n'
        
        #outfile.write(title + '\n')
        rating = pq(ratingElements[count])(".sprite-rating_s_fill").attr('alt')
        stringToWrite += 'rating: ' + rating + '\n'
        #outfile.write(rating + '\n')
        dateStr = pq(ratingElements[count])(".ratingDate").attr('title')
        stringToWrite += 'Date: ' + dateStr + '\n'
        #outfile.write(dateStr + '\n')
        textStr = pq(ratingElements[count])(".entry").text()
        stringToWrite += 'review: ' + textStr + '\n'
        #outfile.write(textStr + '\n')
        roomTip = getTextIfItIsThere(pq(ratingElements[count])('.reviewItem'))
        stringToWrite += 'room tip: ' + roomTip + '\n'
        #outfile.write(roomTip + '\n')
        managementResponse = getTextIfItIsThere(pq(ratingElements[count])(".mgrRspnInline"))
        stringToWrite += 'management response: ' + managementResponse + '\n'
        
        traveledAs = getTextIfItIsThere(pq(ratingElements[count])('.recommend-titleInline'))
        stringToWrite += 'traveled as: ' + traveledAs + '\n'
        #outfile.write(traveledAs + '\n')
        
        # open the file where to write the data and close the file.
        finalStr = (stringToWrite+'\n\n').encode('utf8')
...
def getTAReviewsForHotel( revUrl, city, key):
    ...
    while count < totalpg:
        ...
        if dwnld:
            analyzeReviewPageModified(hotelid, fileContentStr,  title, nextpath)
        count = count + 1
    return
